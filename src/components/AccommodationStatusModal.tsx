...
const AccommodationStatusModal: React.FC<AccommodationStatusModalProps> = ({
  isOpen,
  onClose,
  accommodation,
  onStatusChange,
}) => {
  const [selectedStatus, setSelectedStatus] = useState<"hidden" | "active" | "booked">(accommodation.status);
          ...
          {statusOptions.map((option) => (
            <div 
              ...
              className={`p-4 border rounded-lg cursor-pointer transition-colors ${option.hoverColor} ${
                selectedStatus === option.status ? 'border-blue-500' : 'border-gray-200'
              }`}
              onClick={() => {
